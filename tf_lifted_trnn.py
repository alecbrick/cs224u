...
class TfTreeRNNClassifier(TfModelBase):
    ...
    def build_graph(self):
        ...
        self.b_comb1 = self.weight_init(self.hidden_dim, self.hidden_dim, 'b_comb1')
        ...
        def combine_children(left_tensor, right_tensor):
            ...
            H_inner = tf.tanh(tf.tensordot(left_H, self.W_comb, [2, 0]) + self.b_comb1)
            # H_inner is now batch - size [?, d, d]
        ...
        def loop_body(node_tensors, i):
            ...
            node_tensor = tf.where(
                ...
                tf.cond(tf.equal(i, 0),
                    ...
                    lambda: combine_children(node_tensors.gather(left_child),
